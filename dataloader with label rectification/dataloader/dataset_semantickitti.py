...
class voxel_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(data) == 3:
            xyz, voxel_label, instance_label = data
        elif len(data) == 4:
            xyz, voxel_label, instance_label, sig = data
            if len(sig.shape) == 2: sig = np.squeeze(sig)
        elif len(data) == 5:
            xyz,  voxel_label, instance_label, sig, origin_len = data
            if len(sig.shape) == 2: sig = np.squeeze(sig)
        else:
        ...
        # torch.save(
        #     {'voxel_label_org': voxel_label,
        #      'voxel_label_rect': processed_label,
        #     },
        # f"label_{index}.pt" 
        # )
        ...
        if self.return_test:
            data_tuple += (grid_ind, voxel_label, return_fea, index)
        else:
            data_tuple += (grid_ind, voxel_label, return_fea)
